...
```bash
./dev-pipeline/run-recovery.ps1                    # Auto-detect and recover
./dev-pipeline/run-recovery.ps1 detect              # Detection report only
./dev-pipeline/run-recovery.ps1 run --dry-run       # Generate prompt, don't execute
./dev-pipeline/run-recovery.ps1 run --yes           # Skip confirmation
./dev-pipeline/run-recovery.ps1 run --model <model> # Override AI model
```
...
```bash
python .claude/command-assets/recovery-workflow/scripts/detect-recovery-state.py
```
...
AskUserQuestion:
  ...
  options:
    - label: "Run recovery script (Recommended)"
      description: "Execute ./dev-pipeline/run-recovery.ps1 — autonomously completes ALL remaining phases (review, commit, merge, etc.) via a dedicated AI session with explicit phase instructions"
...
Or with options:
  ./dev-pipeline/run-recovery.ps1 run --dry-run    # Preview the recovery prompt first
  ./dev-pipeline/run-recovery.ps1 run --yes         # Skip confirmation
  ./dev-pipeline/run-recovery.ps1 run --model <model>  # Specify AI model
```
