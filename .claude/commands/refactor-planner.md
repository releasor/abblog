...
5. **Always validate output via script**:
   - Run:
     ```bash
     python .claude/command-assets/refactor-planner/scripts/validate-and-generate-refactor.py validate --input <output-path>
     ```
...
```bash
python .claude/command-assets/refactor-planner/scripts/validate-and-generate-refactor.py validate --input .prizmkit/plans/refactor-list.json
```
