...
1. **Auto-generate** `browser_interaction` for ALL qualifying features. Read `.claude/command-assets/feature-planner/references/browser-interaction.md` for the object format, field rules, and `tool` selection guide.
...
```bash
python .claude/command-assets/feature-planner/scripts/validate-and-generate.py validate --input <output-path> --mode <new|incremental>
```
