...
describe("GET /api/user/ai-settings", () => {
  ...
  it("returns AI settings", async () => {
    ...
    expect(data.aiApiKey).toBe("sk-test");
    expect(data.aiModel).toBe("gpt-4");
    expect(res.headers.get("Cache-Control")).toContain("private");
  });

  it("returns empty strings when user has no settings", async () => {
    mockGetServerSession.mockResolvedValue({ user: { id: 1 } });
    mockGetAuthUserId.mockReturnValue(1);
...
describe("PATCH /api/user/ai-settings", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await PATCH(makeRequest("http://localhost:3000/api/user/ai-settings", { method: "PATCH", body: JSON.stringify({ aiModel: "gpt-4" }) }));
    expect(res.status).toBe(401);
  });
  ...
  it("updates AI settings", async () => {
    ...
    const res = await PATCH(makeRequest("http://localhost:3000/api/user/ai-settings", { method: "PATCH", body: JSON.stringify({ aiApiKey: "sk-new", aiModel: "gpt-4" }) }));
    const data = await res.json();
    ...
    expect(mockPrisma.user.update).toHaveBeenCalledWith({
      where: { id: 1 },
      data: { aiApiKey: "sk-new", aiApiUrl: null, aiModel: "gpt-4" },
    });
  });
