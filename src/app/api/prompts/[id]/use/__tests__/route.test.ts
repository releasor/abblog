...
describe("POST /api/prompts/[id]/use", () => {
  ...
  it("returns 401 when not authenticated", async () => {
    ...
    const res = await POST(makeRequest("http://localhost:3000/api/prompts/1/use"), { params: Promise.resolve({ id: "1" }) });
  ...
  it("returns 404 when prompt not found", async () => {
    ...
    const res = await POST(makeRequest("http://localhost:3000/api/prompts/999/use"), { params: Promise.resolve({ id: "999" }) });
    expect(res.status).toBe(404);
  });

  it("increments usage count successfully", async () => {
    mockGetServerSession.mockResolvedValue({ user: { id: 1 } });
    mockGetAuthUserId.mockReturnValue(1);
    mockPrisma.prompt.findFirst.mockResolvedValue({ id: 1, userId: 1, title: "Test" });
    mockPrisma.prompt.update.mockResolvedValue({ id: 1, usageCount: 6 });
