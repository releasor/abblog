...
5. **Always validate output via script**:
   - Run:
     ```bash
     python ${SKILL_DIR}/scripts/validate-and-generate-refactor.py validate --input <output-path>
     ```
...
```bash
python ${SKILL_DIR}/scripts/validate-and-generate-refactor.py validate --input .prizmkit/plans/refactor-list.json
```
